...
```
dbx execute --cluster-name=<name of interactive cluster> --job=erictome_cicd_databricks_aws-sample-integration-test
```
...
- To trigger the release pipeline, get the current version from the `erictome_cicd_databricks_aws/__init__.py` file and tag the current code version:
```
